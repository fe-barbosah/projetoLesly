		...
		Petal.prototype = {
			...
			// Hibiscus petals: mais largas e arredondadas
			v3 = v1.clone().mult(this.stretchA * 1.5); // pétala mais larga
			v4 = v2.clone().mult(this.stretchB * 1.5);

        ctx.save();
        ctx.strokeStyle = this.bloom.c;
        ctx.fillStyle = this.bloom.c;
        ctx.beginPath();
        ...
        // Miolo amarelo no centro
        if (this.bloom.petals.indexOf(this) === 0) {
            ctx.save();
            ctx.beginPath();
            ctx.arc(0, 0, this.bloom.r * 0.25, 0, 2 * Math.PI, false);
            ctx.fillStyle = 'rgba(255, 215, 0, 0.8)'; // amarelo ouro
            ctx.fill();
            ctx.restore();
        }
    },
